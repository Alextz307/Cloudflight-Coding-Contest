...
int32_t main() {
    ...
    fin.tie(nullptr);
    fout.tie(nullptr);
    int mx_pow, mx_bill, mx_min;
    fin >> mx_pow >> mx_bill >> mx_min;
    int N;
    fin >> N;
    vector < int > a(N + 1);
    for(int i = 1; i <= N; ++i)
    ...
    vector < int > sol[M + 1], used(N + 1), cnt(N + 1);
    for(int i = 1; i <= M; ++i) {
        ...
        for(int j = 0; j < (int)v.size() && cat > 0; ++j)
            if(used[v[j].second] == 0 && cnt[v[j].second] < mx_min) {
        ...
        if(rest > 0)
            for(int j = 0; j < (int)v.size(); ++j)
                if(used[v[j].second] + rest <= mx_pow && !ok[v[j].second] && cnt[v[j].second] < mx_min) {
                    sol[id].emplace_back(v[j].second - 1);
